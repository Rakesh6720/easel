...
const getResourceIcon = (resourceType: string) => {
  ...
  if (resourceType.includes('insights')) return <BarChart3 className="h-6 w-6" />
...
export default function RecommendationsPage() {
  ...
  const [recommendations, setRecommendations] = useState<AzureResourceRecommendation[]>([])
  ...
  const selectedResourcesData = recommendations.filter(r => selectedResources.includes(r.id))
  const totalMonthlyCost = selectedResourcesData.reduce((sum, resource) => sum + resource.estimatedMonthlyCost, 0)
  ...
  const handleProvision = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to provision resources')
      }

      // Redirect to project detail page
      window.location.href = `/projects/${params.id}`
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to provision resources')
    } finally {
    ...
    return (
      <div className="p-6 space-y-6">
        <div className="flex items-center space-x-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href={`/projects/${params.id}`} className="hover:text-azure-blue">Project Details</Link>
          <span>/</span>
        ...
        <Card className="min-h-[400px] flex items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground max-w-md">
              Our AI is analyzing your project requirements and generating optimized Azure resource recommendations...
            </p>
          </div>
        </Card>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <Card className="min-h-[400px] flex items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <h3 className="text-lg font-semibold text-red-600">Error Loading Recommendations</h3>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Azure Resource Recommendations</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {recommendations.map((resource) => (
            <Card key={resource.id} className={`transition-all duration-200 ${
              selectedResources.includes(resource.id) ? 'ring-2 ring-azure-blue border-azure-blue' : ''
            }`}>
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <Badge variant={
                              resource.priority === 'High' ? 'destructive' :
                              resource.priority === 'Medium' ? 'secondary' : 'outline'
                            }>
                              {resource.priority} Priority
                            </Badge>
                          </div>
                          <p className="text-sm text-muted-foreground">{resource.sku} • {resource.location}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold text-lg">{formatCurrency(resource.estimatedMonthlyCost)}</p>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-azure-blue">{resource.justification}</p>
                    </div>
          ...
          {recommendations.length === 0 && (
            <Card>
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">No Recommendations Generated</h3>
                <p className="text-muted-foreground">
                  Unable to generate recommendations for this project. Try regenerating or check your project requirements.
                </p>
              </CardContent>
            </Card>
          )}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border-t pt-2">
                <div className="flex justify-between font-semibold">
                  ...
                  <span className="text-azure-blue">{formatCurrency(totalMonthlyCost)}</span>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center">
                <p className="text-3xl font-bold text-azure-blue">{selectedResources.length}</p>
                <p className="text-sm text-muted-foreground">resources selected</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-azure-gradient-subtle border-azure-blue/20">
            <CardContent className="p-4">
              <p className="text-sm text-gray-700">
                <strong>Note:</strong> These recommendations are based on your project requirements and 
                current Azure best practices. You can modify configurations before provisioning or add 
