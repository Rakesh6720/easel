...
export default function SettingsPage() {
  ...
  const handleDeleteCredential = async (credentialId: number) => {
    try {
      ...
      const confirmationResponse = await azureService.deleteCredential(credentialId, false)
  ...
  const getStatusBadge = (credential: AzureCredential) => {
    const isRecentlyValidated = new Date(credential.lastValidated) > new Date(Date.now() - 24 * 60 * 60 * 1000)
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">No Azure subscriptions</h3>
              ...
              <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-muted-foreground">
                          <div>
                            <span className="font-medium">Subscription:</span> {credential.subscriptionName || 'Loading...'}
                          </div>
                          <div>
                            <span className="font-medium">ID:</span> 
                            <code className="ml-1 text-xs">{credential.subscriptionId.substring(0, 8)}...</code>
                          </div>
                          ...
                          <div className="flex items-center">
                            <CheckCircle className="mr-1 h-3 w-3" />
                            <span>Validated {formatDate(credential.lastValidated)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleValidateCredentials(credential.id)}
                ...
                <div className="mt-6 p-4 bg-azure-gradient-subtle rounded-lg">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-azure-blue mb-1">Need help with Azure setup?</h4>
                      <p className="text-sm text-muted-foreground">
                        Learn how to create service principals and manage Azure resources in our documentation.
                      </p>
                      <Button variant="ghost" size="sm" className="mt-2 p-0 h-auto text-azure-blue hover:text-azure-blue">
                        View Documentation →
                      </Button>
                    </div>
                  </div>
